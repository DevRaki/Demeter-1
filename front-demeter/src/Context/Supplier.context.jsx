...
import { getSupplierRequest, getSupplieRequest, createSupplierRequest, disableSupplierRequest, updateSupplierRequest, deleteSupplierRequest } from '../Api/Supplier.request.js'
...
export const useSupplier = () => {
    ...
    if (!context)
        throw new Error("Ha ocurrido un error con el uso del contexto de los proveedores");
...
export function Supplier({ children }) {
            ...
            if (res.status === 200) {
                setSupplier((prevSupplier) =>
                    prevSupplier.map((supply) =>
                        supply.ID_Supplier === id ? { ...supply, State: !supply.State } : supply
                    )
                );
            }
        } catch (error) {
    ...
    const deleteSupplier = async (id) => {
        try {
            ...
            if (res.status === 204) setSupplier(supplier.filter(supplier => supplier.ID_Supplier !== id))
        } catch (error) {
            console.log(error);
        }
